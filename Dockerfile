...
# Install packages
RUN apt-get update && \
    apt-get install -y --no-install-recommends dovecot-core dovecot-imapd dovecot-pop3d bsd-mailx apache2 libapache2-mod-php php-dom php-imap php-mbstring php-sqlite3 libmail-imapclient-perl libio-socket-ssl-perl libdatetime-format-mail-perl libgetopt-long-descriptive-perl bzip2 cron && \
    apt-get clean && \
